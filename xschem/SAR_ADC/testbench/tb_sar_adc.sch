...
N -630 -435 -630 -395 {lab=#net1}
N -505 -435 -505 -395 {lab=#net2}
N -505 -435 -500 -435 {lab=#net2}
...
plot \{(((V(d11)/1.8)*2048 + (V(d10)/1.8)*1024 + (V(d9)/1.8)*512 + (V(d8)/1.8)*256 + (V(d7)/1.8)*128 + (V(d6)/1.8)*64 + (V(d5)/1.8)*32 + (V(d4)/1.8)*16 + (V(d3)/1.8)*8 + (V(d2)/1.8)*4 + (V(d1)/1.8)*2 + (V(d0)/1.8)) - 2048)/2048\} rst eoc vinp-vinn
...
.endc
"}
C {devices/vsource.sym} -630 -365 2 0 {name=VI value="pulse(250m \{vdiff/2\} 0.1n 100p 100p \{stoptime\} \{stoptime+1\} 1)"}
...
C {devices/lab_pin.sym} -690 -435 0 0 {name=l5 lab=vinp}
...
C {devices/lab_pin.sym} -440 -435 0 1 {name=l4 lab=vinn}
...
C {devices/vsource.sym} -505 -365 0 0 {name=VIN3 value="pulse(250m \{vdiff/2\} 0.1n 100p 100p \{stoptime\} \{stoptime+1\} 1)"}
...
C {devices/vsource.sym} -510 -160 0 1 {name=V2 value="PULSE(0 1.8 0 100p 100p 2n 30n )"}
...
C {res.sym} -660 -435 1 0 {name=R2
value=0.001
footprint=1206
device=resistor
m=1}
C {res.sym} -470 -435 1 0 {name=R1
value=0.001
footprint=1206
device=resistor
m=1}
